...
pub async fn hiyoko_bingo(ctx: &Context, msg: &Message){
    ...
    let slot_row = 5;
    let slot_column = 5;
    let emoji_str_list= vec!["<:bingo_close:823544230546374676>","<:bingo_open:823544270271021088>","<:bingo_atari:823911268729356328>","<:bingo_free:823542644062814238>"];

    let mut rand_vec = gen_rand_vec(slot_row*slot_column,2);
    let rand_vec = bingo_check(&mut rand_vec,slot_row,slot_column);
    let result_srting = gen_string(rand_vec.to_vec(), emoji_str_list, slot_row, slot_column);
    post::post_message(&ctx,&msg,result_srting).await;
}
...
fn bingo_check(rand_vec: &mut Vec<u8>, row: u8, column:u8) -> &mut Vec<u8>{
    ...
    for number in 0..row{
        //あとで何とかする
        let is_bingo = isnotzero(vec![rand_vec[(row*0+number) as usize],rand_vec[(row*1+number) as usize],rand_vec[(row*2+number) as usize],rand_vec[(row*3+number) as usize],rand_vec[(row*4+number) as usize]]);
        if is_bingo == true {
            println!("tate bingo");
            //あとで何とかする
            for n in 0..column{
    ...
    for number in 0..column{
        ...
        let is_bingo = isnotzero(vec![rand_vec[(0+row*number) as usize],rand_vec[(1+row*number) as usize],rand_vec[(2+row*number) as usize],rand_vec[(3+row*number) as usize],rand_vec[(4+row*number) as usize]]);
        if is_bingo == true {
            println!("yoko bingo");
            //あとで何とかする
            for n in 0..row{
    ...
    //斜め方向チェック
    //あとで何とかする
    let is_bingo = isnotzero(vec![rand_vec[0],rand_vec[6],rand_vec[12],rand_vec[18],rand_vec[24]]);
    if is_bingo == true {
    ...
    //あとで何とかする
    let is_bingo = isnotzero(vec![rand_vec[4],rand_vec[8],rand_vec[12],rand_vec[16],rand_vec[20]]);
    if is_bingo == true {
        println!("naname bingo");
        //あとで何とかする
        for n in 0..row{
...
fn isnotzero(vec_num: Vec<u8>) -> bool{
...
pub async fn hiyoko_bowling(ctx: &Context, msg: &Message){
    println!("Shard {}", ctx.shard_id);
    let emoji_str_list= vec![":bowling:",":basketball:",":football:",":flying_disc:",":fishing_pole_and_fish:",":curling_stone:",":boxing_glove:",":boomerang:"];
    let buon_emoji_str = "<:buon:788397267882344468>";
    let dunk_hiyoko_emoji_str = "<:dunk:811008417337704498>";

    let mut rand_vec = gen_rand_vec(1,emoji_str_list.len());
    let mut result_string = String::new();
    if rand_vec[0] == 1 {
